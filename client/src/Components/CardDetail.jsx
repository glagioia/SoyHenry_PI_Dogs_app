    ...
    if(match.params.id.length > 11){
        ...
        for(let i = 0; i < filtrados.length; i++){    
            if(filtrados[i].id === Number(match.params.id)){
                 image = filtrados[i].image.url;
                 name = filtrados[i].name;
                 height = filtrados[i].height.metric;
                ...
                if(filtrados[i].weight.metric === undefined){
                    if(filtrados[i].weight.metric === undefined){
                         ...
                         weight = filtrados[i].weight.join(" - ");
                    }
                }else{
                     weight = filtrados[i].weight.metric;
                }
                 life_span = filtrados[i].life_span;
                 temperaments = filtrados[i].temperament;
        ...
        <Fragment>
            <div className="containerMayor">
                ...
                <span className="containerDetail">
                    ...
                    <label className="labelDetail">Altura:</label> <label className="labelDetail">{height} cm</label> <br />
